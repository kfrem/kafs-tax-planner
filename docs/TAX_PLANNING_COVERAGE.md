...
| SDLT reliefs (multiple/mixed-use/uninhabitable) | ✓ | SDLT built with the additional-dwelling surcharge + FTB relief; **mixed-use now built** — `sdlt-mixed-use-classification` (FA 2003 s.55(1B) Table B, flagged borderline): residential-with-surcharge vs Table B, quantified. Remaining: uninhabitable/derelict (case-law-driven, rare). (Multiple Dwellings Relief was abolished June 2024.) | Low |
