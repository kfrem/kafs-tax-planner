...
| Incorporation of property portfolios | ✓ | `property-incorporation` — **the flagship**: compares the personal s.24 tax on the rental business against corporation tax (interest fully deductible, profits retained), weighs the annual saving against the one-off SDLT-on-transfer (at additional-dwelling rates) and CGT (deferred where s.162 relief applies), and reports the **break-even in years** (TCGA 1992 s.162). v1 covers an England/SDLT portfolio with profits retained; extraction/ATED/LBTT-LTT variants are documented next steps. Flagged borderline (s.162 'business' test). | — |
