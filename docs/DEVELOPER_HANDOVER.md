...
| F8 | Disposal-composition golden case | A new GoldenTestCase `source` string exceeded `varchar(255)`, so `seed_rule_base` raised `DataError` and every test using the `seeded_rule_base` fixture *errored* (not failed) | Shortened the source under the limit; the suite caught it immediately via the fixture. Lesson: golden `source`/`description` are DB-bounded (255) — keep the working terse, or the seed fails before any assertion runs |
