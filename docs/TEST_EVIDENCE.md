...
**Expected result: `226 passed`** (≈6 minutes; the seed fixture rebuilds
...
| `ruleengine/tests/test_planning_reliefs.py` | 6 | Tier-1 planning strategies. **Gift Aid** (`TestGiftAid`): higher-rate donor gets the rate difference (£800 net → £1,000 gross → £200 personal relief), a donation restores the personal allowance in the £100k–125k taper (£8k net → £4,000 relief, 40% effective, £5,000 PA restored), and a basic-rate donor gets nothing extra. **Directors' loan s.455** (`TestDirectorsLoanS455`): the 33.75% charge on the balance outstanding after the 9-month window (£50k, £20k repaid → £10,125), fully repaid avoids it (£13,500 avoided), nothing repaid (£25k → £8,437.50) — all hand-computed |
| `ruleengine/tests/test_golden_cases.py` | 2 (data-driven) | Executes all **35 GoldenTestCase rows stored in the rule base itself** (each with its own `tax_year`, so 2026/27 and 2024/25 cases run against those years' rows) — the §5.5 release gate. Adding a golden case to the seed automatically adds coverage |
