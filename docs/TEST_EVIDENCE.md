...
**Expected result: `270 passed`** (≈6 minutes; the seed fixture rebuilds
...
| `ruleengine/tests/test_golden_cases.py` | 2 (data-driven) | Executes all **51 GoldenTestCase rows stored in the rule base itself** (each with its own `tax_year`, so 2026/27 and 2024/25 cases run against those years' rows) — the §5.5 release gate. Adding a golden case to the seed automatically adds coverage |
...
| `advice/tests/test_intake.py` | 6 | **Guided intake** (`advice/intake.py`): `intake_gaps` surfaces the material questions the engine would otherwise assume — marital status/spouse income (always asked; recording 0 answers it), property jurisdiction (only for a transaction; England assumed), landlord mortgage/s.24, BPR qualification, pension taper, partnership commerciality, trust prior transfers — each with why it matters and the assumption made; a fully-recorded simple client has no gaps. Shown on the advice page as "Questions to confirm before relying on this advice" |
