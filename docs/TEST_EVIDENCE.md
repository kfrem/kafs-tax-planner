...
**Expected result: `200 passed`** (≈4½ minutes; the seed fixture rebuilds
...
| `ruleengine/tests/test_golden_cases.py` | 2 (data-driven) | Executes all **26 GoldenTestCase rows stored in the rule base itself** (each with its own `tax_year`, so a 2026/27 case runs against the 2026/27 rows) — the §5.5 release gate. Adding a golden case to the seed automatically adds coverage |
